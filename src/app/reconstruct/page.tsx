        ...
        return (
          <React.Fragment key={s.key}>
            <div className="flex flex-col items-center gap-1.5 flex-1">
              ...
              >
                {isCompleted ? (
                  <svg className="w-3 h-3 text-[#616c39]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="3">
...
function ReconstructContent() {
  ...
  // Fetch user plots when logged in
  useEffect(() => {
    if (!currentUser) {
      setUserPlots([]);
      setSelectedPlotId("");
    ...
    const fetchPlots = async () => {
      try {
        ...
        if (plotsRes.ok) {
          ...
          const fetched = plotsData.plots || [];
          setUserPlots(fetched);
          if (fetched.length > 0 && !selectedPlotId) {
            // Keep default empty or first plot
          }
        }
      } catch (err) {
    ...
    fetchPlots();
  }, [currentUser, phase, currentScan?.id]);
    ...
    try {
      const r = await fetch(`${BACKEND_URL}/reconstruct`, {
        ...
        body: JSON.stringify({
          ...
          plot_id: selectedPlotId ? parseInt(selectedPlotId) : null,
        }),
      });
      if (!r.ok) {
      ...
      <div className="fixed inset-0 bg-white overflow-hidden font-sans text-[#292524] flex flex-col pt-14 sm:pt-16">
        ...
        <div className="flex-1 flex flex-col lg:flex-row relative overflow-hidden">
          ...
          {currentScan && renderViewer ? (
            <iframe
              src={`${BACKEND_URL}/viewer.html?v=26&code=${currentScan.tree_code}&url=${encodeURIComponent(currentScan.splat_file_url)}&proxy=true`}
              ...
              <div className="bg-white/95 backdrop-blur-xl border border-[#e7e5e4]/80 rounded-2xl shadow-xl px-5 py-3 flex flex-col sm:flex-row items-center justify-between gap-4">
                  ...
                  <div className="text-xs text-[#292524] font-medium mt-0.5">
                    {currentScan.plot_id ? (
                      <span className="text-[#616c39] flex items-center gap-1.5 font-medium">
                        <svg className="w-3.5 h-3.5 text-[#616c39] shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2.5">
                          <path strokeLinecap="round" strokeLinejoin="round" d="M4.5 12.75l6 6 9-13.5" />
                        </svg>
                        {language === "id" ? `Terklaim ke Plot #${currentScan.plot_id}` : `Claimed to Plot #${currentScan.plot_id}`}
                      </span>
                      ...
                      <span className="text-[#616c39] flex items-center gap-1.5 font-medium">
                        ...
                        {language === "id" ? "Tersimpan di Pohon Saya (Belum masuk plot)" : "Saved to My Trees (Not in a plot yet)"}
                      </span>
                    ) : (
                      <span>{language === "id" ? "Belum diklaim ke plot mana pun" : "Not claimed to any plot yet"}</span>
                    )}
                  </div>
                </div>

                {!currentScan.plot_id && (!currentScan.claimed_by_user_id || currentScan.claimed_by_user_id === currentUser.id) && userPlots.length > 0 && (
                  <div className="flex items-center gap-3 w-full sm:w-auto">
                    ...
                    >
                      {claiming ? (language === "id" ? "Mengklaim..." : "Claiming...") : (language === "id" ? "Klaim ke Plot" : "Claim to Plot")}
                    </button>
                  </div>
                )}
                ...
                {!currentScan.plot_id && (!currentScan.claimed_by_user_id || currentScan.claimed_by_user_id === currentUser.id) && userPlots.length === 0 && (
                  <div className="text-xs text-[#79716b] italic">
                    {language === "id" ? "Anda belum memiliki plot." : "You don't have any plots yet."}{" "}
        ...
        >
          ...
          <div className="px-6 pt-6 pb-4 border-b border-[#fafaf9] flex items-center justify-between bg-white">
            ...
            <button
              ...
              className="w-8 h-8 rounded-full bg-[#fafaf9] text-[#79716b] hover:bg-[#e7e5e4] hover:text-[#292524] flex items-center justify-center transition"
            >
              ...
              <div className="flex flex-col gap-2 mt-2">
                {isEditing3D ? (
                  ...
                  <>
                    ...
                    {/* Preliminary Notice if Uncalibrated Scale or Insufficient Height */}
                    {currentScan && (currentScan.scale_status === 'uncalibrated' || currentScan.height_used !== 'full_height' || (currentScan.tinggi_m && currentScan.tinggi_m < 1.3)) && (
                      <div className="w-full p-2.5 bg-amber-500/10 border border-amber-500/20 rounded-xl flex items-start gap-2">
              ...
              <div className="space-y-2 bg-[#fafaf9] border border-[#e7e5e4] rounded-2xl p-4">
                ...
                <div className="space-y-2">
                  {currentScan.scale_status !== "calibrated" && (
                    <div className="text-[11px] text-red-750 bg-red-50 border border-red-200 rounded-xl p-2.5 leading-relaxed flex items-start gap-2">
                      <svg className="w-4 h-4 text-red-600 shrink-0 mt-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2.5">
                        <path strokeLinecap="round" strokeLinejoin="round" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
                      </svg>
                      <div>
                  ...
                  {currentScan.quality_status && currentScan.quality_status !== "ok" && currentScan.quality_status !== "failed" && (() => {
                    ...
                    return (
                      <div className="text-[11px] text-amber-700 bg-amber-50 border border-amber-200 rounded-xl p-2.5 leading-relaxed flex items-start gap-2">
        ...
        {error && (
          <div className="absolute bottom-6 left-1/2 -translate-x-1/2 z-50 flex items-center gap-3 bg-[#292524] text-[#ffffff] text-xs px-5 py-3 rounded-xl shadow-lg">
            <span>{error}</span>
            <button onClick={() => setError(null)} className="opacity-60 hover:opacity-100 transition p-0.5">
              <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2.5">
                <path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" />
              </svg>
            </button>
            ...
            <>
              ...
              <div className="bg-white border border-[#e7e5e4] rounded-3xl p-6 sm:p-8 shadow-sm">
                <form onSubmit={handleSubmit} className="flex flex-col gap-5">
                  ...
                  {currentUser && userPlots.length > 0 && (
                    <div className="flex flex-col gap-1.5">
                      <label className="text-[10px] font-bold uppercase tracking-wider text-[#79716b]">
                        {language === "id" ? "Plot Hutan Tujuan" : "Target Forest Plot"}{" "}
                        <span className="normal-case tracking-normal font-normal text-[#d6d3d1]">
                          {language === "id" ? "(opsional)" : "(optional)"}
                        </span>
                      </label>
                      <select
                        ...
                        onChange={(e) => setSelectedPlotId(e.target.value)}
                        disabled={loading}
                        className="px-4 py-2.5 rounded-xl bg-[#fafaf9] border border-[#e7e5e4] focus:outline-none focus:border-[#a8a29e] text-xs font-medium transition cursor-pointer text-[#292524]"
                      >
                        <option value="">
                          {language === "id" ? "— Simpan ke Pohon Saya (tanpa plot khusus) —" : "— Save to My Trees (no specific plot) —"}
                        </option>
                        {userPlots.map((p) => (
                          <option key={p.id} value={p.id}>
                            {p.name} ({p.plot_code})
